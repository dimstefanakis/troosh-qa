...
import useGetMyAssignedQuestions from "./hooks/useGetMyAssignedQuestions";

function MyAssignedQuestions() {
  const query = useGetMyAssignedQuestions();
  if (query.isLoading) {
          ...
          return (
            <React.Fragment key={question.id}>
              <Flex flexFlow="column" mb={10}>
                ...
                {/**
                 * If it's a future date show the below
                 * else show a "delivered at" section
                 */}
                {question.zoom_link && (
                  <Link href={question.zoom_link} passHref>
                    <ChakraLink color="teal.500" href={question.zoom_link} target="_blank" marginTop="10px">
                      Zoom link (starts <Text as="b">{dateNoTime + " " + deliveryFormatted + "-"}</Text>)
                    </ChakraLink>
                  </Link>
                )}
                {/* <Flex marginTop="10px">
